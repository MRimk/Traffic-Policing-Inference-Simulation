...
ThroughputMonitor(Ptr<PacketSink> sink, double interval)
{
    ...
    double throughput = ((bytesReceived - lastBytes) * 8) / interval; // Convert to bps
    lastBytes = bytesReceived;

    // Log time and throughput to file
    throughputFile << Simulator::Now().GetSeconds() << " " << throughput / 1e6 << std::endl;
...
main(int argc, char* argv[])
{
    ...
    pointToPoint.SetDeviceAttribute("DataRate", StringValue("20Mb/s")); // link bandwidth
    ...
    double throughput = (totalBytesReceived * 8) / simulationTime; // Convert to bits per second
